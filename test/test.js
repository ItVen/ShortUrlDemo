...
describe('server', function () {
    ...
    describe('测试首页路由', function () {
        it('打开首页', function (done) {
            request.get({ url: 'http://localhost:8009' }, function (error, response, body) {
    ...
    describe('测试提交一条链接', function () {
        it('返回一条短链接', function (done) {
            request.post('http://localhost:8009', {form: {url: 'https://gitee.com/eclipes/projects'}}, function (error, response, body) {
    ...
    describe('测试访问短链接', function () {
        it('重定向到映射的长链接', function (done) {
            ...
            u.save(function(err,urls){
                if(urls){
                    request.get({
                        url: 'http://localhost:8009/testurl',
    ...
    describe('测试一个不存在的链接', function () {
        it('应返回404错误', function (done) {
            request.get({
                url: 'http://localhost:8009/nonexistenturl',
                followRedirect: false
            }, function (error, response, body) {
    ...
    describe('测试查看所有短链的接口', function () {
        it('返回所有数据', function (done) {
            request.post('http://localhost:8009/api/all',{form: {url: 'http://as.dds.hs/sdhjas'}},function (error, response, body) {
                expect(response.statusCode).to.equal(200);
    ...
    describe('测试查看一条短链的接口', function () {
        it('返回一条短链接', function (done) {
            request.post('http://localhost:8009/api/find',{form: {url: 'http://as.dds.hs/sdhjas'}},function (error, response, body) {
                expect(response.statusCode).to.equal(200);
                expect(body).to.include('"status":1,');
                done();
            });
    ...
    describe('测试生成短链的接口', function () {
        it('返回一条短链接', function (done) {
            request.post('http://localhost:8009/api/short',{form: {url: 'https://gitee.com/eclipes/projects'}},function (error, response, body) {
                expect(response.statusCode).to.equal(200);
                expect(body).to.include('"status":1,');
                done();
            });
